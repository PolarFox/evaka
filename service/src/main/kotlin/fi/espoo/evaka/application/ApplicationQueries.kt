    ...
    val conditions =
        listOfNotNull(
            ...
            if (authorizedUnitsForApplicationsWithoutAssistanceNeed != AclAuthorization.All)
                "((a.document->'careDetails'->>'assistanceNeeded')::boolean = true OR a.preferredUnits && :authorizedUnitsForApplicationsWithoutAssistanceNeed)"
            else null,
            if (authorizedUnitsForApplicationsWithAssistanceNeed != AclAuthorization.All)
                "((a.document->'careDetails'->>'assistanceNeeded')::boolean = false OR a.preferredUnits && :authorizedUnitsForApplicationsWithAssistanceNeed)"
            else null,
            if (
            ...
            when (voucherApplications) {
                ...
                VoucherApplicationFilter.VOUCHER_ONLY ->
                    "a.preferredUnits && (SELECT array_agg(id) FROM daycare WHERE provider_type = 'PRIVATE_SERVICE_VOUCHER')"
                VoucherApplicationFilter.NO_VOUCHER ->
                    "NOT a.preferredUnits && (SELECT array_agg(id) FROM daycare WHERE provider_type = 'PRIVATE_SERVICE_VOUCHER')"
                null -> null
            }
        )
